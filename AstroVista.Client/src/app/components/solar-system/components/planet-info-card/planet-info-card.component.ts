  ...
  template: `
    ...
    >
      ...
      <div class="text-sm">
        ...
        @if (body?.diameter) {
          <p class="my-1.5">
            <span class="text-ebony-200 font-semibold">Diameter:</span> {{ body?.diameter | number }} km
          </p>
        }
        @if (body?.distanceFromSun) {
          <p class="my-1.5">
            <span class="text-ebony-200 font-semibold">Distance from Sun:</span>
            {{ body?.distanceFromSun | number }} million km
          </p>
        }
        @if (body?.orbitalPeriod) {
          <p class="my-1.5">
            <span class="text-ebony-200 font-semibold">Orbital Period:</span> {{ body?.orbitalPeriod }} days
          </p>
        }
        @if (body?.rings) {
